...
def parse_monkeys(input):
    monkeys = []
    for i in range(int((len(input) + 1) / 7)):
        starting_line = i * 7
        starting_items = [int(item) for item in input[starting_line + 1][18:].split(", ")]
        operation = (lambda x, y: x * y) if input[starting_line + 2][23:24] == "*" else (lambda x, y: x + y)
        operation_amount = input[starting_line + 2][25:]
        test = int(input[starting_line + 3][21:])
        test_true = int(input[starting_line + 4][29:])
        test_false = int(input[starting_line + 5][30:])
